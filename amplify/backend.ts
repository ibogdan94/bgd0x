...
// Sources live under media/*, rendered clips under renders/*. Grant both.
for (const prefix of ["media/*", "renders/*"]) {
  mediaBucket.grantReadWrite(backend.video.resources.lambda, prefix);
  mediaBucket.grantDelete(backend.video.resources.lambda, prefix);
}
