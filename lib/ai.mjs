...
import { dedupeBatch, isDuplicate } from "./dedupe.mjs";
...
function topicBlock(topics = []) {
  ...
  return [
    ...
    ...titles.map((t) => `- ${t}`),
    "",
    "Ground the tweets in THIS news cycle. Each tweet should riff on a DIFFERENT item above",
    "(or a real, specific thing it reminds you of). Have a take on it — never summarize the",
    "headline, never quote it verbatim, and don't mention that you read it in the news.",
  ].join("\n");
}
...
async function generateDistinct({ n, avoid, build, log }) {
  const kept = [];
  let priors = [...avoid];
  ...
  for (let attempt = 0; attempt < 2 && kept.length < n; attempt++) {
    ...
    const { kept: fresh, rejected } = dedupeBatch(batch, priors);
    kept.push(...fresh);
    priors = [...priors, ...batch];
...
export async function generateTweets(n = 5, { avoid = [], topics = [], log } = {}) {
  ...
  return generateDistinct({ n, avoid, build, log });
}

// Generate N degen tweets riffing on a specific topic / keyword.
export async function draftTopicTweets(topic, n = 5, { avoid = [], log } = {}) {
  const conviction = convictionAllowed(n);
  const build = (need, priors) => {
    const angles = pickAngles(need);
    return sections(
