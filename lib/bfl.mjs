...
// pass `keyframes` (an array of URLs/base64) instead to drive one scene through
// 2-3 stills so the model animates BETWEEN them (e.g. a pose-to-pose turn).
// Returns { id, polling_url }.
export async function createVideoI2V({
  image,
  keyframes,               // optional: array of images the scene passes through
  prompt,
  duration = 5,
  ...
  const frames = keyframes?.length ? keyframes : [image];
  const body = {
    mode: "i2v",
    prompt,
    keyframes: frames,      // 1 image → animate from it; 2-3 → interpolate through
    duration,
    resolution,
