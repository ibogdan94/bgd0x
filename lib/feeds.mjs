...
import { clusterTopics } from "./dedupe.mjs";
...
export async function fetchHeadlines({
  ...
  // Ten feeds cover the same stories, so the exact-title check above is not
  // enough — CoinDesk and Cointelegraph both carried the Metaplanet reward-pool
  // cut under different words. Collapse to one headline per story.
  const unique = clusterTopics(merged, (x) => x.title);
  log?.(`Headlines: ${merged.length} fetched, ${unique.length} distinct stories`);
