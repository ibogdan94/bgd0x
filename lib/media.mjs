...
// Stable mascot reference stills (uploaded to media/mascots/ by
// scripts/import-mascot-refs.mjs). EVERY generated clip attaches these so the
// bgd0x mascot is always the visual identity sent to BFL.
export const MASCOT_REF_KEYS = [1, 2, 3, 4, 5].map((n) => `media/mascots/${n}.png`);
// Curated 2-3 pose combos that read as a natural turn / reveal in i2v.
const MASCOT_COMBOS = [
  ["media/mascots/4.png", "media/mascots/2.png", "media/mascots/3.png"], // profile → 3/4 → front, arms crossed
  ["media/mascots/2.png", "media/mascots/3.png"],                        // side → front
  ["media/mascots/5.png", "media/mascots/3.png"],                        // hand-to-chest → arms crossed
  ["media/mascots/4.png", "media/mascots/3.png"],                        // profile → front
  ["media/mascots/1.png"],                                               // full-body stride
];
export function pickMascotKeyframes() {
  return MASCOT_COMBOS[Math.floor(Math.random() * MASCOT_COMBOS.length)];
}
...
export async function getSignedUrlFor(key, { expiresIn = 3600, downloadName } = {}) {
