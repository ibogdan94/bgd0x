...
// Posts per day is randomized between these two so the cadence doesn't look
// mechanical. The scheduler Lambda runs every 15 min and posts at most one due
// slot, so the ceiling here is what actually governs daily volume.
const MIN_GAP = Number(process.env.MIN_GAP_MINUTES || 40); // minutes between posts
const MIN_POSTS = Number(process.env.POSTS_PER_DAY_MIN || 5);
const MAX_POSTS = Number(process.env.POSTS_PER_DAY_MAX || 6);
