...
export async function generateFromSource(source, n = 5) {
  ...
  if (source.type === "rss") {
    ...
    const items = clusterTopics(parseFeed(await res.text(), 20), (x) => x.title);
  ...
  // topic source
  const avoid = await recentPosted(30).catch(() => []);
  const tweets = await draftTopicTweets(source.value, n, { avoid });
  const drafts = tweets.map((t) => ({ type: "content", text: t.text, source: null, mode: "queue" }));
  await addDrafts(drafts);
  return drafts.length;
}
