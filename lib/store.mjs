...
import { QUEUE, STATE, DRAFTS, POSTED, NEWS_SEEN, TOPICS, SOURCES, MEDIA, ENV } from "./paths.mjs";
...
export async function deleteDraft(id) {
  if (useDynamo) { await del("draft", id); return; }
  const d = readJSON(DRAFTS, { drafts: [] });
  d.drafts = d.drafts.filter((x) => x.id !== id);
  writeJSON(DRAFTS, d);
}
// Remove drafts created more than `days` ago so stale news doesn't pile up.
// Returns the number removed.
export async function purgeOldDrafts(days = 7) {
  ...
  if (useDynamo) {
    let removed = 0;
    for (const d of await query("draft")) {
      if (isOld(d)) { await del("draft", d.sk); removed++; }
    }
    return removed;
  }
  const store = readJSON(DRAFTS, { drafts: [] });
  const before = store.drafts.length;
  store.drafts = store.drafts.filter((d) => !isOld(d));
  writeJSON(DRAFTS, store);
...
// ---------- Topics covered (one tweet per news story) ----------
// Headlines the account has already tweeted about. Short-lived on purpose: a
// story is fair game again once it stops being the same news cycle.
const TOPIC_DAYS = Number(process.env.TOPIC_MEMORY_DAYS || 3);

export async function recentTopics(days = TOPIC_DAYS) {
  const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
  const fresh = (at) => {
  ...
  if (useDynamo) {
    return (await query("topic")).filter((i) => fresh(i.at)).map((i) => i.text).filter(Boolean);
  }
  return readJSON(TOPICS, []).filter((i) => fresh(i.at)).map((i) => i.text).filter(Boolean);
}

export async function addTopics(headlines = []) {
  const clean = headlines.filter(Boolean).map((h) => String(h).slice(0, 300));
  if (!clean.length) return;
  const at = new Date().toISOString();
  if (useDynamo) {
    const ttl = Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 14;
    for (const text of clean) await put({ pk: "topic", sk: nextSeq(), text, at, ttl });
    return;
  }
  const cur = readJSON(TOPICS, []);
  clean.forEach((text) => cur.push({ text, at }));
  writeJSON(TOPICS, cur.slice(-300));
}
...
// ---------- Media (image metadata for the video pipeline) ----------
// Bytes live in a folder / S3 (see media.mjs); this tracks only metadata.
export async function listMedia() {
  if (useDynamo) return query("media");
  return readJSON(MEDIA, { media: [] }).media;
}
export async function getMedia(id) {
  if (useDynamo) return get("media", id);
  return readJSON(MEDIA, { media: [] }).media.find((m) => m.id === id);
}
export async function addMediaMeta(item) {
  const rec = { id: `m_${Date.now()}_${seq++}`, status: "pending", createdAt: new Date().toISOString(), ...item };
  if (useDynamo) { await put({ pk: "media", sk: rec.id, ...rec }); return rec; }
  const d = readJSON(MEDIA, { media: [] });
  d.media.push(rec);
  writeJSON(MEDIA, d);
  return rec;
}
export async function updateMedia(id, patch) {
  if (useDynamo) {
    const cur = (await get("media", id)) || { pk: "media", sk: id };
    await put({ ...cur, ...patch });
    return;
  }
  const d = readJSON(MEDIA, { media: [] });
...
export async function deleteMediaMeta(id) {
  if (useDynamo) { await del("media", id); return; }
  const d = readJSON(MEDIA, { media: [] });
...
// Most recent posted tweets, newest first. Feeds the generator's "don't repeat
// yourself" check — the sk is an ISO timestamp, so a descending query is enough.
export async function recentPosted(limit = 20) {
  if (useDynamo) {
    const items = await query("posted", { Limit: limit, ScanIndexForward: false });
    return items.map((i) => i.text).filter(Boolean);
  }
  if (!existsSync(POSTED)) return [];
  return readFileSync(POSTED, "utf8")
    .split("\n")
    .filter((l) => l.trim())
    .slice(-limit)
    .reverse()
