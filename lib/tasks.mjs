...
import { hasAI, generateTweets, draftNewsReaction, draftVideoPost, pickSceneIdea } from "./ai.mjs";
import {
...
export async function runVideoPost(log = console.log) {
  ...
  const { video, caption, motionPrompt, keyframes, scene } = await renderMascotVideo({}, log);
  const rec = await addMediaMeta({
    contentType: "image/png", ext: "png", label: "weekly auto mascot",
    status: "preview", caption, motionPrompt, scene, aspect: "1:1", keyframes, key: keyframes[0],
  });
...
export async function renderMascotVideo({ text = "", keyframes } = {}, log = console.log) {
  ...
  let scene = text;
  if (!scene) {
    const used = (await listMedia()).map((m) => m.scene).filter(Boolean);
    scene = pickSceneIdea(used);
  }
  log(`[${stamp()}] Video: rendering mascot clip (${keys.length} keyframe(s)) — "${scene.slice(0, 70)}"…`);
  ...
  return { video, caption, motionPrompt, keyframes: keys, scene };
}
...
export async function generateMascotPreview(text = "", log = console.log) {
  const { video, caption, motionPrompt, keyframes, scene } = await renderMascotVideo({ text }, log);
  const rec = await addMediaMeta({
