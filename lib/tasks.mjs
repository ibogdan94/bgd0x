...
export async function runScheduler(log = console.log) {
  ...
  if (!item) {
    log(`[${stamp()}] Slot ${due[0].label} due but queue is EMPTY.`);
    return { posted: false, emptyQueue: true };
  }

  const data = await postTweet(item.text);
  const url = `https://x.com/i/status/${data.id}`;
  await popQueue(item.id);
  due[0].posted = true;
  due[0].url = url;
  await setState(state);
  await logPosted(item.text, url);
  log(`[${stamp()}] Posted (${due[0].label}): ${url} | ${await countQueue()} left`);
