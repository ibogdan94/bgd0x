...
import {
  ...
  addDrafts, seenHas, seenAdd, purgeOldDrafts, listMedia,
} from "./store.mjs";
import { hasBFL, renderVideoFromImage } from "./bfl.mjs";
import { nextUnusedMedia, getImageBase64, markUsed } from "./media.mjs";

const stamp = () => new Date().toISOString();

// ---- Video: animate one uploaded image via BFL, auto-post to X ----
...
const VIDEO_DAILY_MAX = Number(process.env.VIDEO_DAILY_MAX || 2);
const localHour = () =>
...
export async function runVideoPost(log = console.log) {
  ...
  const usedToday = (await listMedia()).filter(
    (m) => m.status === "used" && m.usedAt && localDateString(new Date(m.usedAt)) === today
  ).length;
  if (usedToday >= VIDEO_DAILY_MAX) { log(`[${stamp()}] Video: daily cap ${VIDEO_DAILY_MAX} reached`); return { posted: false, reason: "daily cap" }; }

  const item = await nextUnusedMedia();
  if (!item) { log(`[${stamp()}] Video: no unused images`); return { posted: false, reason: "no unused images" }; }
  return renderAndPost(item, log);
}

// Render one image → video and post it. Shared by the scheduler and the manual
// "animate & post" button. Bypasses window/cap (caller decides).
export async function renderAndPost(item, log = console.log) {
  ...
  const image = await getImageBase64(item);
  const video = await renderVideoFromImage({ image, prompt: motionPrompt, duration: 5, resolution: "hd", aspectRatio: "9:16" });

  const data = await postTweetWithVideo(caption, video);
  const url = `https://x.com/i/status/${data.id}`;
  await markUsed(item.id, { videoUrl: url, caption });
