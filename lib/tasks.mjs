...
async function autoTweet(log) {
  if (hasAI()) {
    try {
      // Ground the tweet in the current news cycle and in what's already been
      // posted, so back-to-back auto posts don't rhyme with each other.
      const [topics, avoid] = await Promise.all([
        fetchHeadlines({ perFeed: 2, limit: 10, log: (m) => log(`[${stamp()}] ${m}`) }).catch(() => []),
        recentPosted(20).catch(() => []),
      ]);
      const t = await generateTweets(1, { avoid, topics, log: (m) => log(`[${stamp()}] ${m}`) });
      if (t && t[0]) return t[0];
...
export async function runGenerate(n = 6, log = console.log) {
  ...
  if (hasAI()) {
    try {
      ...
      log(`Drawing topics from ${topics.length} live headline(s).`);
      tweets = await generateTweets(n, { avoid, topics, log });
  ...
  if (!tweets || !tweets.length) {
    ...
    while (tweets.length < n && src.length)
      tweets.push(src.splice(Math.floor(Math.random() * src.length), 1)[0]);
  }
